...
from nomura_corporation_app.models import Client
...
from finbaapp.models import Client
...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		print('Script to top up clients automatically')
		for client in Client.objects.annotate(pro= F('deposit') * F('roi') + F('profit'), bal= F('deposit') + F('profit')):
			if float(client.running_days) == float(client.contract_duration):
				client.profit= 0
				client.deposit= 0
				client.balance= float(client.balance) + client.bal
				client.save()
				...
				template= render_to_string('finbaapp/email_top_up_complete.html', {'name':client_username})
				...
				client_username= client.user.username
				client_email= client.user.email
				credit_template= render_to_string('finbaapp/email_top_up.html', {'name':client_username})
