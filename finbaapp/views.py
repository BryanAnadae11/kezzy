...
def leadership(request):
	return render(request, 'finbaapp/leadership.html')

def home(request):
	return render(request, 'finbaapp/index.html')

def plan(request):
	return render(request, 'finbaapp/plan.html')

def otherpayment(request):
    return render(request, 'finbaapp/otherpayment.html')


def about(request):
	return render(request, 'finbaapp/company.html')


def news(request):
	return render(request, 'finbaapp/blog-details.html')


def career(request):
	return render(request, 'finbaapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('finbaapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'finbaapp/login.html')


def privacy(request):
	return render(request, 'finbaapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'finbaapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('finbaapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('finbaapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to TradeSavor',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('finbaapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with TradeSavor!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@Tradesavor.org'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'finbaapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'finbaapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'finbaapp/adminpage.html', context)


def terms(request):
	return render(request, 'finbaapp/terms.html')


def realestate(request):
	return render(request, 'finbaapp/realestate.html')


def crypto(request):
	return render(request, 'finbaapp/crypto.html')


def forex(request):
	return render(request, 'finbaapp/forex.html')


def nft(request):
	return render(request, 'finbaapp/nft.html')


def contact(request):
	return render(request, 'finbaapp/contact.html')


def stocks(request):
	return render(request, 'finbaapp/stocks.html')


def agriculture(request):
	return render(request, 'finbaapp/agriculture.html')


def gold(request):
	return render(request, 'finbaapp/gold.html')


def retirement(request):
	return render(request, 'finbaapp/retirement.html')

@login_required(login_url='signin')
def investmentPlans(request):
	return render(request, 'finbaapp/investmentplans.html')
...
def retirementDeposit(request):
	if request.method == 'POST':
		...
		print(price_amount)
		print(pay_currency)
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/retirementdepositForm.html', context)
...
def realestateDeposit(request):
	if request.method == 'POST':
		...
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/realestateDepositForm.html', context)
...
def cryptoDeposit(request):
	if request.method == 'POST':
		...
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/cryptoDepositForm.html', context)
...
def stocksDeposit(request):
	if request.method == 'POST':
		...
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/StocksDepositForm.html', context)
...
def portfolioDeposit(request):
	if request.method == 'POST':
		...
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/PortfolioDepositForm.html', context)

@login_required(login_url='signin')
def trialplan(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		if investment_info and price_amount and pay_currency:
		    ...
		    return HttpResponse('Please fill form correctly before submitting')
	context= {}
	return render(request, 'finbaapp/trialplan.html', context)
...
def deposit(request):
	...
	if price_amount and pay_currency:
		...
		payload=json.dumps({
			...
			"success_url": "https://www.Tradesavor.org/dashboard",
			#our success url will direct us to the get_payment_status view for balance top ups
			"cancel_url": "https://www.Tradesavor.org/dashboard"
		})
		headers={'x-api-key':'B699DMC-CAXMKXN-N8W3PGT-PXP3JQM', 'Content-Type': 'application/json'}
		...
		template= render_to_string('finbaapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
		plain_message= strip_tags(template)
		...
		emailmessage.send()
		try:
			send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@Tradesavor.org'])
		except BadHeaderError:
			pass
		return redirect(generated_link)
		context={}
	return render(request, 'finbaapp/deposittwo.html', context)
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@Tradesavor.org'])
	...
	context={}
	return render(request, 'finbaapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'finbaapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'finbaapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'finbaapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'finbaapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'finbaapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'finbaapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'finbaapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('finbaapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('finbaapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'finbaapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('finbaapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'finbaapp/account_settings.html', context)
...
def notifications(request):
	...
	return render(request, 'finbaapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('finbaapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('finbaapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@Tradesavor.org'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'finbaapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'finbaapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('finbaapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	return HttpResponse('Contact accountofficer@Tradesavor.org for reinvestments')
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@Tradesavor.org'])
	...
	return render(request, 'finbaapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'finbaapp/portfolio.html', context)